...
| 角色 | 说明 | 权限 |
|------|------|------|
| `USER` | 普通用户 | 查看宠物、点赞、添加自己的宠物等 |
| `ADMIN` | 管理员 | 拥有所有权限，包括后台管理功能 |
...
登录成功后，前端需要将 token 存储到 localStorage，并在后续请求中通过请求头携带 token。
...
系统使用 Sa-Token 框架进行认证，部分接口需要登录后才能访问：

| 接口类型 | 是否需要登录 | 说明 |
|----------|-------------|------|
| 公开接口 | 否 | 宠物列表、详情、排行榜、相册浏览等 |
| 需要登录 | 是 | 添加/编辑/删除宠物、点赞、添加相册等 |
| 仅管理员 | 是 + ADMIN角色 | 后台管理相关接口 |
